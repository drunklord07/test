...
description="AWS Audit for SNS Topics Publicly Accessible for Publishing"
criteria="Identifies SNS topics with policies that allow public ('*') publishing access."
...
# Function to check SNS Topics for public access
    ...
    if [[ -n "$topics" ]]; then
        ...
        for topic_arn in $topics; do
            ...
            if [[ -n "$policy" && "$policy" != "None" ]]; then
                ...
                action=$(echo "$policy" | grep -oP '"Action":\s*"\K[^"]+')
                condition_present=$(echo "$policy" | grep -oP '"Condition":')

                if [[ "$effect_allow" == "Allow" && "$principal" == "*" && "$action" == "SNS:Publish" && -z "$condition_present" ]]; then
...
for REGION in $regions; do
    check_sns_public_access "$REGION" &
done
...
# Audit Section
echo -e "${PURPLE}Listing SNS Topics with Publicly Accessible Publishing...${NC}"
...
for region in "${!non_compliant_topics[@]}"; do
    ...
    for topic in "${topics_in_region[@]}"; do
        ...
        echo -e "${RED}Status: NON-COMPLIANT (Publicly Accessible for Publishing)${NC}"
        echo "----------------------------------------------------------------"
    done
done

if [[ "$non_compliant_found" == false ]]; then
    echo -e "${GREEN}All SNS topics have restricted publishing access.${NC}"
fi
