...
description="AWS Audit for Publicly Accessible SQS Queues"
criteria="Identifies SQS queues that have an IAM policy allowing unrestricted public access."
...
# Function to check SQS Public Access
check_sqs_public_access() {
    ...
    if [[ -n "$queues" ]]; then
        ...
        for queue_url in $queues; do
            ...
            if [[ -n "$policy_json" && "$policy_json" == *'"Effect": "Allow"'* && "$policy_json" == *'"Principal": "*"'* && "$policy_json" != *'"Condition"'* ]]; then
...
for REGION in $regions; do
    check_sqs_public_access "$REGION" &
done
...
# Audit Section
echo -e "${PURPLE}Listing Publicly Accessible SQS Queues...${NC}"
...
if [[ "$non_compliant_found" == false ]]; then
    echo -e "${GREEN}All SQS queues are private and secure.${NC}"
fi
