...
description="AWS Audit: S3 Bucket Public READ_ACP ACL Exposure"
criteria="This script checks if any S3 bucket has READ_ACP permission set for public access (AllUsers group)."
...
for BUCKET in $BUCKETS; do
  ...
  if echo "$OUTPUT" | grep -q '"Permission": "READ_ACP"'; then
...
if [ ${#NON_COMPLIANT_BUCKETS[@]} -eq 0 ]; then
  ...
  echo "Non-Compliant S3 Buckets (Public READ_ACP ACL):"
