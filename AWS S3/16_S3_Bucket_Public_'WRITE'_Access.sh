...
description="AWS Audit: S3 Bucket Public WRITE ACL Exposure"
criteria="This script checks if any S3 bucket has WRITE permission set for public access (AllUsers group)."
...
for BUCKET in $BUCKETS; do
  ...
  if echo "$OUTPUT" | grep -q '"Permission": "WRITE"'; then
...
if [ ${#NON_COMPLIANT_BUCKETS[@]} -eq 0 ]; then
  ...
  echo "Non-Compliant S3 Buckets (Public WRITE ACL):"
