...
description="AWS Audit: S3 Bucket ACL for Authenticated Users (WRITE_ACP Access)"
criteria="This script checks if any S3 bucket grants WRITE_ACP access to the 'Authenticated Users' group."
...
# Audit S3 Bucket ACLs for WRITE_ACP Access
compliant_count=0
...
check_acl() {
  ...
  if echo "$acl_output" | grep -q "WRITE_ACP"; then
    echo "$bucket|Non-Compliant (WRITE_ACP access granted to Authenticated Users)" >> "$lock_file"
  else
...
while IFS= read -r entry; do
  ...
  case "$reason" in
    "Non-Compliant (WRITE_ACP access granted to Authenticated Users)")
...
echo "-------------------------------------------------------------------------------"
printf "${GREEN}%-30s${NC} %-15s %-40s\n" "Compliant" "$compliant_count" "Buckets without WRITE_ACP access for Authenticated Users"
printf "${RED}%-30s${NC} %-15s %-40s\n" "Non-Compliant" "$non_compliant_count" "WRITE_ACP access granted to Authenticated Users"
echo "-------------------------------------------------------------------------------"
