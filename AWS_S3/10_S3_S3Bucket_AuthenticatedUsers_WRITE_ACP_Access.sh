...
# Check bucket ACL for Authenticated Users group (WRITE_ACP Access)
...
for BUCKET in $BUCKETS; do
  ...
  if echo "$OUTPUT" | grep -q "WRITE_ACP"; then
...
if [ ${#NON_COMPLIANT_BUCKETS[@]} -eq 0 ]; then
  ...
  echo -e "${RED}Non-Compliant S3 Buckets (WRITE_ACP Access for Authenticated Users):${NC}"
