...
description="AWS Audit: S3 Bucket Public ACL Exposure"
criteria="This script checks if any S3 bucket has FULL_CONTROL permission set for public access (AllUsers group)."
...
for BUCKET in $BUCKETS; do
  ...
  if echo "$OUTPUT" | grep -q '"Permission": "FULL_CONTROL"'; then
...
if [ ${#NON_COMPLIANT_BUCKETS[@]} -eq 0 ]; then
  ...
  echo "Non-Compliant S3 Buckets (Public FULL_CONTROL ACL):"
