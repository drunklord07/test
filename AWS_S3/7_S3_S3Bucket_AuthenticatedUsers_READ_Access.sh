...
description="AWS Audit: S3 Bucket ACL for Authenticated Users (READ Access)"
criteria="This script checks if any S3 bucket grants READ access to the 'Authenticated Users' group."
...
# Audit S3 Bucket ACLs for READ Access
compliant_count=0
...
check_acl() {
  ...
  if echo "$acl_output" | grep -q "READ"; then
    echo "$bucket|Non-Compliant (READ access granted to Authenticated Users)" >> "$lock_file"
  else
...
while IFS= read -r entry; do
  ...
  case "$reason" in
    "Non-Compliant (READ access granted to Authenticated Users)")
...
echo "-------------------------------------------------------------------------------"
printf "${GREEN}%-30s${NC} %-15s %-40s\n" "Compliant" "$compliant_count" "Buckets without READ access for Authenticated Users"
printf "${RED}%-30s${NC} %-15s %-40s\n" "Non-Compliant" "$non_compliant_count" "READ access granted to Authenticated Users"
echo "-------------------------------------------------------------------------------"
