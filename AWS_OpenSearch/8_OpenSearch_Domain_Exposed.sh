...
description="AWS Audit for Publicly Accessible OpenSearch Domains"
criteria="Checks if OpenSearch domains have an access policy allowing public access (Principal: '*')."
...
check_opensearch_domains() {
    ...
    if [[ -n "$domain_names" ]]; then
        for DOMAIN in $domain_names; do
            ...
            policy_json=$(aws es describe-elasticsearch-domain --region "$REGION" --profile "$PROFILE" --domain-name "$DOMAIN" --query 'DomainStatus.AccessPolicies' --output text 2>/dev/null)

            if [[ -n "$policy_json" ]] && echo "$policy_json" | grep -q '"AWS": "\*"' && echo "$policy_json" | grep -q '"Effect": "Allow"'; then
                public_domain_list["$DOMAIN"]="NON-COMPLIANT (Publicly Accessible)"
            fi
        done
    fi
...
for region in "${!domain_mappings[@]}"; do
    ...
    for domain in "${!domains_in_region[@]}"; do
        ...
        echo -e "${RED}Status: NON-COMPLIANT (Publicly Accessible)${NC}"
        echo "----------------------------------------------------------------"
    done
done

if [[ "$non_compliant_found" == false ]]; then
    echo -e "${GREEN}No publicly accessible OpenSearch domains found. All domains are COMPLIANT.${NC}"
fi
