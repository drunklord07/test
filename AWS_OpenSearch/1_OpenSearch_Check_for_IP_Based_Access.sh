...
# Function to check OpenSearch domain policies
check_domain_policies() {
    ...
    for DOMAIN in $domain_names; do
        ...
        if [[ -n "$access_policy" ]]; then
            access_policies["$DOMAIN"]="$access_policy"
        fi
    done

    total_domains["$REGION"]=$checked_count
    policy_mappings["$REGION"]=$(declare -p access_policies)
