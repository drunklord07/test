...
description="AWS Audit for OpenSearch Domain In-Transit Encryption"
criteria="This script checks if Amazon OpenSearch domains have in-transit encryption (HTTPS enforcement) enabled."

# Commands used
command_used="Commands Used:
  1. aws es list-domain-names --region \$REGION --query 'DomainNames[*].DomainName'
  2. aws es describe-elasticsearch-domain --region \$REGION --domain-name \$DOMAIN --query 'DomainStatus.DomainEndpointOptions.EnforceHTTPS'"
...
check_in_transit_encryption() {
    ...
    for DOMAIN in $domain_names; do
        ...
        encryption_enabled=$(aws es describe-elasticsearch-domain --region "$REGION" --profile "$PROFILE" --domain-name "$DOMAIN" --query 'DomainStatus.DomainEndpointOptions.EnforceHTTPS' --output text 2>/dev/null)
...
for REGION in $regions; do
    check_in_transit_encryption "$REGION" &
done
...
echo -e "${PURPLE}Checking for OpenSearch domains without in-transit encryption...${NC}"
...
for region in "${!encryption_status[@]}"; do
    ...
    for domain in "${!domains_in_region[@]}"; do
        ...
        # Check if encryption is disabled (false)
    ...
    if [[ ${#unencrypted_domains[@]} -gt 0 ]]; then
        ...
        echo "OpenSearch Domains without In-Transit Encryption (HTTPS not enforced):"
...
if [[ "$violation_found" = false ]]; then
    echo -e "${GREEN}All OpenSearch domains have in-transit encryption enabled. No issues found.${NC}"
fi
