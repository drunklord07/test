...
description="AWS Audit for Unrestricted Outbound Rules in Network ACLs (NACLs)"
criteria="This script identifies NACLs that have an ALLOW rule without a defined 'PortRange' attribute, meaning all outbound traffic is allowed, making them non-compliant."
...
for REGION in "${!nacl_counts[@]}"; do
  if [ "${nacl_counts[$REGION]}" -gt 0 ]; then
    ...
    for NACL_ID in $(aws ec2 describe-network-acls --region "$REGION" --profile "$PROFILE" \
      ...
      # Get outbound ALLOW rules without PortRange
      nacl_entries=$(aws ec2 describe-network-acls --region "$REGION" --profile "$PROFILE" \
        --network-acl-ids "$NACL_ID" --query 'NetworkAcls[*].Entries[?(RuleAction==`allow`) && (Egress==`true`) && !not_null(PortRange)]' \
        --output json)
