...
description="AWS Audit for EKS Cluster Kubernetes Secrets Encryption"
criteria="This script checks whether Kubernetes secrets encryption using AWS KMS CMKs is enabled for each Amazon EKS cluster."

# Commands used
command_used="Commands Used:
  1. aws eks list-clusters --region \$REGION --query 'clusters' --output text
  2. aws eks describe-cluster --region \$REGION --name \$CLUSTER --query 'cluster.encryptionConfig[*].provider.keyArn' --output text"
...
for REGION in $regions; do
  ...
  for CLUSTER in $clusters; do
    ...
    # Get encryption key ARN for Kubernetes secrets
    key_arn=$(aws eks describe-cluster --region "$REGION" --profile "$PROFILE" \
      --name "$CLUSTER" --query 'cluster.encryptionConfig[*].provider.keyArn' --output text)

    if [[ -z "$key_arn" || "$key_arn" == "None" ]]; then
      non_compliant_list+=("$CLUSTER (Secrets Encryption Disabled)")
    fi
  done
...
if [ ${#non_compliant_clusters[@]} -gt 0 ]; then
  ...
  echo -e "${GREEN}All EKS clusters have Kubernetes secrets encryption enabled.${NC}"
fi
