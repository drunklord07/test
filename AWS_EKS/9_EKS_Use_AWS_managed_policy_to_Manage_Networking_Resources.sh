...
description="AWS Audit for EKS Node Group IAM Role Compliance"
criteria="This script checks if the AmazonEKS_CNI_Policy is attached to each EKS cluster node group IAM role."
...
# Required policy
REQUIRED_POLICY="arn:aws:iam::aws:policy/AmazonEKS_CNI_Policy"
  ...
  for CLUSTER in $clusters; do
    ...
    for NODEGROUP in $nodegroups; do
      ...
      if [[ ! " ${policies[@]} " =~ " $REQUIRED_POLICY " ]]; then
        non_compliant_list+=("$CLUSTER / $NODEGROUP (IAM Role: $role_name - Missing AmazonEKS_CNI_Policy)")
      fi
    done
  done
...
if [ ${#non_compliant_nodegroups[@]} -gt 0 ]; then
  ...
  echo -e "${GREEN}All EKS node groups have the required AmazonEKS_CNI_Policy.${NC}"
fi
