...
description="AWS Audit for Security Groups Allowing Unrestricted HTTPS Access"
criteria="This script checks if any EC2 security groups allow inbound HTTPS (TCP port 443) traffic from 0.0.0.0/0."

# Commands used
command_used="Commands Used:
  1. aws ec2 describe-regions --query 'Regions[*].RegionName' --output text
  2. aws ec2 describe-security-groups --region \$REGION --filters Name=ip-permission.from-port,Values=443 Name=ip-permission.to-port,Values=443 Name=ip-permission.cidr,Values='0.0.0.0/0' --query 'SecurityGroups[*].{Name:GroupName}'"
...
# Table Header
echo "Region         | Unrestricted HTTPS Security Groups"
echo "+----------------+----------------------------------+"
...
for REGION in $regions; do
  # Get security group names allowing unrestricted HTTPS access
