...
description="AWS Audit for Security Groups Allowing Unrestricted SSH Access"
criteria="This script checks if any EC2 security groups allow inbound SSH traffic (TCP 22) from 0.0.0.0/0."

# Commands used
command_used="Commands Used:
  1. aws ec2 describe-regions --query 'Regions[*].RegionName' --output text
  2. aws ec2 describe-security-groups --region \$REGION --filters Name=ip-permission.from-port,Values=22 Name=ip-permission.to-port,Values=22 Name=ip-permission.cidr,Values='0.0.0.0/0' --output table --query 'SecurityGroups[*].GroupId'"
...
# Table Header
echo "Region         | Unrestricted SSH Security Groups"
echo "+----------------+----------------------------------+"
...
for REGION in $regions; do
  # Get security group IDs allowing unrestricted SSH access
