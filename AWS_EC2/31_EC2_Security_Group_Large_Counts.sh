...
criteria="This script checks the number of EC2 security groups in each AWS region. If the total exceeds 50, action is required to remove unnecessary or overlapping security groups."
...
for REGION in $regions; do
  ...
  # Highlight in red if security groups exceed 50
  if [ "$total_sg_count" -gt 50 ]; then
...
for REGION in "${!region_sg_count[@]}"; do
  if [ "${region_sg_count[$REGION]}" -gt 50 ]; then
    echo -e "${RED}WARNING: Region $REGION has ${region_sg_count[$REGION]} security groups, exceeding the recommended limit of 50.${NC}"
