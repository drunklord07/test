...
description="AWS Audit for Security Groups Allowing Unrestricted ICMP Access"
criteria="This script checks if any EC2 security groups allow inbound ICMP traffic from 0.0.0.0/0."

# Commands used
command_used="Commands Used:
  1. aws ec2 describe-regions --query 'Regions[*].RegionName' --output text
  2. aws ec2 describe-security-groups --region \$REGION --filters Name=ip-permission.protocol,Values=icmp Name=ip-permission.cidr,Values='0.0.0.0/0' --query 'SecurityGroups[*].GroupId' --output text"
...
# Table Header
echo "Region         | Unrestricted ICMP Security Groups"
echo "+----------------+----------------------------------+"
...
for REGION in $regions; do
  # Get security group IDs allowing unrestricted ICMP access
