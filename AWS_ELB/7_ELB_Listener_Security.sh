...
description="AWS Audit for Classic Load Balancer Secure Listener Configuration"
criteria="This script checks if Classic Load Balancers have secure listeners (HTTPS or SSL) enabled."
...
for REGION in $regions; do
  ...
  for LB_NAME in $lb_names; do
    ...
    # Check if load balancer has secure listeners (HTTPS/SSL)
    listener_config=$(aws elb describe-load-balancers --region "$REGION" --profile "$PROFILE" \
    ...
    if echo "$listener_config" | grep -q '"ListenerDescriptions": \[\]'; then
...
if [ ${#non_compliant_lbs[@]} -gt 0 ]; then
  ...
  echo -e "${GREEN}All Classic Load Balancers have secure HTTPS/SSL listeners.${NC}"
fi
