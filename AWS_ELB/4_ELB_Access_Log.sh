...
description="AWS Audit for Access Logging on Classic Load Balancers"
criteria="This script checks if Classic Load Balancers have Access Logging enabled."
...
command_used="Commands Used:
  ...
  3. aws elb describe-load-balancer-attributes --region \$REGION --load-balancer-name \$LB_NAME --query 'LoadBalancerAttributes.AccessLog.Enabled' --output text"
...
for REGION in $regions; do
  ...
  for LB_NAME in $lb_names; do
    ...
    access_logging_enabled=$(aws elb describe-load-balancer-attributes --region "$REGION" --profile "$PROFILE" \
      --load-balancer-name "$LB_NAME" \
      --query 'LoadBalancerAttributes.AccessLog.Enabled' --output text)

    # If access logging is not enabled, add to non-compliant list
    if [[ "$access_logging_enabled" != "true" ]]; then
...
if [ ${#non_compliant_lbs[@]} -gt 0 ]; then
  echo -e "${RED}Non-Compliant Load Balancers (Access Logging Disabled):${NC}"
  echo "----------------------------------------------------------------"
  ...
  echo -e "${GREEN}All Classic Load Balancers have Access Logging enabled.${NC}"
fi
