...
description="AWS ACM Issued SSL/TLS Certificates Audit"
criteria="This script lists all issued SSL/TLS certificates in AWS ACM across multiple regions and checks if they are about to expire within the next 45 days.
Certificates expiring in less than 45 days are marked as 'Non-Compliant' (printed in red), otherwise 'Compliant' (printed in green)."
...
expiry_threshold=$((current_timestamp + 3888000))  # 45 days from now
  ...
  if [ "${region_cert_count[$REGION]}" -gt 0 ]; then
    ...
    while read -r cert_arn domain_name; do
      ...
      if [ "$expiry_timestamp" -le "$expiry_threshold" ]; then
        ...
        echo -e "Status: ${RED}Non-Compliant (Expiring in less than 45 days)${NC}"
      else
        echo -e "Expiration Date: $human_readable_expiry"
