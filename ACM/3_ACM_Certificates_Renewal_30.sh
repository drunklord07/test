...
criteria="This script lists all issued SSL/TLS certificates in AWS ACM across multiple regions and checks if they are about to expire within the next 30 days.
Certificates expiring in less than 30 days are marked as 'Non-Compliant' (printed in red), otherwise 'Compliant' (printed in green)."
...
expiry_threshold=$((current_timestamp + 2592000))  # 30 days from now
  ...
  if [ "${region_cert_count[$REGION]}" -gt 0 ]; then
    ...
    while read -r cert_arn domain_name; do
      ...
      if [ "$expiry_timestamp" -le "$expiry_threshold" ]; then
        ...
        echo -e "Status: ${RED}Non-Compliant (Expiring in less than 30 days)${NC}"
      else
        echo -e "Expiration Date: $human_readable_expiry"
