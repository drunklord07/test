...
description="AWS Audit for Lambda Functions Using Function URLs"
criteria="This script checks all AWS Lambda functions across regions to verify whether they are configured with Function URLs."

# Commands used
command_used="Commands Used:
  1. aws lambda list-functions --region \$REGION --query 'Functions[*].FunctionName'
  2. aws lambda list-function-url-configs --region \$REGION --function-name \$FUNCTION --query 'FunctionUrlConfigs[*].FunctionUrl'"
...
for REGION in $regions; do
  ...
  for FUNCTION in $function_names; do
    ...
    url_config=$(aws lambda list-function-url-configs --region "$REGION" --profile "$PROFILE" --function-name "$FUNCTION" --query 'FunctionUrlConfigs[*].FunctionUrl' --output json 2>/dev/null)
...
if [ ${#non_compliant_functions[@]} -gt 0 ]; then
  echo -e "${RED}Non-Compliant Lambda Functions (No Function URL Configured):${NC}"
  echo "----------------------------------------------------------------"
  ...
  echo -e "${GREEN}All AWS Lambda functions have Function URLs configured.${NC}"
fi
