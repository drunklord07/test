...
description="AWS Audit for Lambda Function URLs (IAM Authentication)"
criteria="This script checks if AWS Lambda function URLs have IAM authentication enabled."

# Commands used
command_used="Commands Used:
  1. aws lambda list-functions --region \$REGION --query 'Functions[*].FunctionName'
  2. aws lambda list-function-url-configs --region \$REGION --function-name \$FUNCTION --query 'FunctionUrlConfigs[*].AuthType'"
...
for REGION in $regions; do
  ...
  for FUNCTION in $function_names; do
    ...
    # Get Function URL Authentication Type
    auth_type=$(aws lambda list-function-url-configs --region "$REGION" --profile "$PROFILE" --function-name "$FUNCTION" --query 'FunctionUrlConfigs[*].AuthType' --output text 2>/dev/null)
...
if [ ${#non_compliant_functions[@]} -gt 0 ]; then
  echo -e "${RED}Non-Compliant Lambda Functions (Publicly Accessible Function URLs):${NC}"
  echo "----------------------------------------------------------------"
  ...
  echo -e "${GREEN}All AWS Lambda function URLs have IAM authentication enabled.${NC}"
fi
