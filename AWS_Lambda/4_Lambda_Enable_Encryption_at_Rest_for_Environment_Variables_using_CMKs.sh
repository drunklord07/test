...
description="AWS Audit for Lambda Environment Variable Encryption"
criteria="This script checks all AWS Lambda functions across regions to verify if they are using a Customer Managed Key (CMK) for environment variable encryption."

# Commands used
command_used="Commands Used:
  1. aws lambda list-functions --region \$REGION --query 'Functions[*].FunctionName'
  2. aws lambda get-function --region \$REGION --function-name \$FUNCTION --query 'Configuration.KMSKeyArn'"
...
for REGION in $regions; do
  ...
  for FUNCTION in $function_names; do
    ...
    # Check Lambda KMS Encryption
    kms_key=$(aws lambda get-function --region "$REGION" --profile "$PROFILE" --function-name "$FUNCTION" --query 'Configuration.KMSKeyArn' --output text 2>/dev/null)
...
if [ ${#non_compliant_functions[@]} -gt 0 ]; then
  echo -e "${RED}Non-Compliant Lambda Functions (No Customer Managed KMS Key Used):${NC}"
  echo "----------------------------------------------------------------"
  ...
  echo -e "${GREEN}All AWS Lambda functions use a Customer Managed KMS Key for encryption.${NC}"
fi
