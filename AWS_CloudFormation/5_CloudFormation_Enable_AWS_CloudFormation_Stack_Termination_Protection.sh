...
description="AWS Audit for CloudFormation Stacks with Termination Protection"
criteria="Verifies whether CloudFormation stacks have Termination Protection enabled to prevent accidental deletions."
...
2. aws cloudformation describe-stacks --region \$REGION --stack-name \$STACK_NAME --query 'Stacks[*].EnableTerminationProtection' --output text"
...
for REGION in "${!total_stacks[@]}"; do
  ...
  for STACK_NAME in $stacks; do
    termination_protection=$(aws cloudformation describe-stacks --region "$REGION" --profile "$PROFILE" --stack-name "$STACK_NAME" --query 'Stacks[*].EnableTerminationProtection' --output text 2>/dev/null)
...
if [ ${#non_compliant_stacks[@]} -gt 0 ]; then
  echo -e "${RED}Non-Compliant CloudFormation Stacks (Termination Protection Disabled):${NC}"
  echo "----------------------------------------------------------------"

  for region in "${!non_compliant_stacks[@]}"; do
    if [[ -n "${non_compliant_stacks[$region]}" ]]; then
      echo -e "${PURPLE}Region: $region${NC}"
      echo "Stacks without Termination Protection:"
  ...
  echo -e "${GREEN}All CloudFormation stacks have Termination Protection enabled.${NC}"
fi
